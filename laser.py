#!/usr/bin/env python 
"""
Tweaked and modified by Dutchpatriot 2017
...
G1 X0 Y0 F5000
""",
'footer': """G1 X0 Y0 Z0 F5000 (Home All Axes)
...
class laser_gcode(inkex.Effect):
    ...
    def export_gcode(self,gcode):
		...
		max_val = float(self.options.stop_travel)
		pas = (float(self.options.start_travel)-(float(x)*float(self.options.pass_depth)))
		pas_tot= (math.ceil(pas*100)/100)
		tot = x+1
		zero_start = float(self.options.start_travel)==0
		if max_val<=pas:
			gcode += "G90\nG1 Z" + str(pas_tot) + "  (Amount of passes " + str(tot) + "/" + str(self.options.passes) + ")" +  "\nG90\n" + gcode_pass
		elif zero_start:
			gcode += "G91\nG1 Z0  (Amount of passes " + str(tot) + "/" + str(self.options.passes) + ")" +  "\nG90\n" + gcode_pass
		else :
			gcode += "G90\nG1 Z" + str(max_val) + "  (Amount of passes " + str(tot) + "/" + str(self.options.passes) + ")" +  "\nG90\n" + gcode_pass
#		else :
#			gcode += "G90\nG1 Z" + str(max_val) + "  (Aantal passes " + str(x) + "/" + str(self.options.passes) + ")" +  #"\nG90\n" + gcode_pass                
		#gcode += "G91\nG1 Z-" + print (p+diepte); + "  (Aantal passes " + str(x) + "/" + str(self.options.passes) + ")" + "\nG90\n" + gcode_pass

            #gcode += "G91\nG1 Z-" + self.options.pass_depth + "\nG90\n" + gcode_pass
        f = open(self.options.directory+self.options.file, "w")
        f.write(self.options.laser_off_command + " S0" + "\n" + self.header + "G1 F" + self.options.travel_speed + "\n" + gcode + self.footer + "(END G-Code, number of passes = " + (str(self.options.passes+1)) + ")")
        f.close()
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--start-travel",                      action="store", type="string",          
dest="start_travel",                          default="10",                            help="Sart -Z axis at hight mm - inch")
        self.OptionParser.add_option("",   "--stop-travel",                      action="store", type="string",          
dest="stop_travel",                          default="10",                            help="Stop -Z axis lowering at mm - inch")
...
#                    print_((-f(w[k][i-1]),-f(w[k][i]), [i1[5] for i1 in l1]) )
    ...
    def check_dir(self):
        ...
        if self.options.start_travel <= 0 :
                self.header += "\nG90 Z-\n"
        elif self.options.start_travel >= 0 :
                self.header += "\nG90 Z" + str(self.options.start_travel) +"\n"
